...
-- インデックスの作成
CREATE INDEX idx_daily_progress_user_date ON daily_learning_progress(user_id, date DESC);
CREATE INDEX idx_daily_progress_date ON daily_learning_progress(date);
CREATE INDEX idx_learning_summaries_user_period ON learning_summaries(user_id, period_type, period_start DESC);
