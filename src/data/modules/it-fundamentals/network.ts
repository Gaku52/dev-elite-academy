...
3. **セキュリティ問題**
  ...
  },
  {
    ...
    sections: [
      {
        title: '情報理論の基礎',
        content: `情報の量を数学的に扱う理論の基礎：

**情報量とエントロピー:**
- 情報量：発生確率の低い事象ほど情報量が大きい
- 情報量 I = -log₂(P) （Pは確率）
- エントロピー：情報源の不確実性の度合い
- H = -Σ P(x) log₂ P(x)
...
- ハミング符号：1ビット誤り訂正`,
        quizzes: [
          {
            question: '確率1/8の事象の情報量は？',
            options: ['1ビット', '3ビット', '8ビット', '1/8ビット'],
            correct: 1,
            explanation: '情報量 I = -log₂(1/8) = -log₂(2⁻³) = 3ビットです。'
          },
          {
            question: 'ハフマン符号の特徴は？',
            options: ['固定長符号', '可変長符号で最適', 'エラー訂正機能', '暗号化機能'],
            correct: 1,
            explanation: 'ハフマン符号は出現頻度に応じた可変長符号で、平均符号長を最小化します。'
          },
          {
            question: 'エントロピーH=0の情報源の特徴は？',
            options: ['完全にランダム', '予測不可能', '確定的（常に同じ記号）', 'エラーが多い'],
            correct: 2,
            explanation: 'エントロピーが0ということは不確実性がなく、常に同じ記号が出力される確定的な情報源です。'
          },
          {
            question: 'CRC（巡回冗長検査）の用途は？',
            options: ['データ圧縮', 'データ暗号化', 'データ誤り検出', 'データソート'],
            correct: 2,
            explanation: 'CRCは送信データに冗長ビットを付加して、受信側で誤りを検出するために使用されます。'
          },
          {
            question: 'シャノンの符号化定理が示すものは？',
            options: ['圧縮の限界', '伝送速度の限界', '暗号の強度', 'ネットワークの帯域'],
            correct: 0,
            explanation: 'シャノンの符号化定理は、可逆圧縮の理論的限界がエントロピーで決まることを示します。'
          },
          {
            question: 'パリティビットで検出できる誤りは？',
            options: ['任意の誤り', '2ビット誤り', '奇数個の誤り', '偶数個の誤り'],
            correct: 2,
            explanation: 'パリティビットは奇数個のビット誤りは検出できますが、偶数個の誤りは検出できません。'
          },
          {
            question: 'ランレングス符号化に適したデータは？',
            options: ['完全にランダムなデータ', '同じ値が連続するデータ', '暗号化されたデータ', '音声データ'],
            correct: 1,
            explanation: 'ランレングス符号化は同じ値が連続して現れるデータに対して効果的な圧縮を行えます。'
          },
          {
            question: 'ハミング距離が3の符号の特徴は？',
            options: ['1ビット誤り検出のみ', '1ビット誤り訂正可能', '2ビット誤り訂正可能', '3ビット誤り訂正可能'],
            correct: 1,
            explanation: 'ハミング距離dの符号では、(d-1)/2ビットまでの誤りを訂正できるため、距離3では1ビット誤り訂正が可能です。'
          },
          {
            question: '算術符号化の利点は？',
            options: ['実装が簡単', 'ハフマン符号より高圧縮率', '高速処理', '誤り耐性'],
            correct: 1,
            explanation: '算術符号化は理論的な圧縮限界により近づけるため、ハフマン符号よりも高い圧縮率を実現できます。'
          },
          {
            question: '相互情報量I(X;Y)=0が意味することは？',
            options: ['XとYは完全に相関', 'XとYは独立', 'Xの情報量が0', 'Yの情報量が0'],
            correct: 1,
            explanation: '相互情報量が0ということは、XとYが統計的に独立で、互いに情報を提供しないことを意味します。'
          }
        ]
      },
      {
        title: '通信理論と変調',
        content: `デジタル通信における信号処理の基礎：
...
**多重化技術:**
- FDM（Frequency Division Multiplexing）：周波数分割
- TDM（Time Division Multiplexing）：時分割
- CDM（Code Division Multiplexing）：符号分割
- OFDM：直交周波数分割多重
...
- シャノン・ハートレーの定理`,
        quizzes: [
          {
            question: 'ナイキスト定理で、4kHzの信号を正しく標本化する最小周波数は？',
            options: ['2kHz', '4kHz', '8kHz', '16kHz'],
            correct: 2,
            explanation: 'ナイキスト定理により、信号の最高周波数の2倍以上で標本化する必要があるため、8kHz以上が必要です。'
          },
          {
            question: 'PSK変調で変化させるパラメータは？',
            options: ['振幅', '周波数', '位相', '電力'],
            correct: 2,
            explanation: 'PSK（Phase Shift Keying）は搬送波の位相を変化させてデジタルデータを伝送する変調方式です。'
          },
          {
            question: '16-QAMで1シンボルあたり何ビット伝送できる？',
            options: ['2ビット', '4ビット', '8ビット', '16ビット'],
            correct: 1,
            explanation: '16-QAMは16通りの状態を持つため、1シンボルで log₂(16) = 4ビットの情報を伝送できます。'
          },
          {
            question: 'OFDMの特徴は？',
            options: ['単一キャリア', '直交する複数キャリア', 'アナログ変調', '時分割多重'],
            correct: 1,
            explanation: 'OFDM（直交周波数分割多重）は、互いに直交する複数のサブキャリアでデータを並列伝送する技術です。'
          },
          {
            question: 'シャノン・ハートレーの定理 C = B log₂(1 + S/N) で、Cは何を表す？',
            options: ['信号電力', '雑音電力', '通信路容量', '帯域幅'],
            correct: 2,
            explanation: 'Cは通信路容量（チャンネル容量）で、理論上の最大伝送レートを表します。'
          },
          {
            question: 'CDMAで使用される拡散符号の性質は？',
            options: ['周期性', '直交性', '単調性', '対称性'],
            correct: 1,
            explanation: 'CDMA（符号分割多元接続）では、直交性を持つ拡散符号を使用して複数ユーザーの信号を分離します。'
          },
          {
            question: 'BER（ビット誤り率）が10⁻⁶の意味は？',
            options: ['1秒間に10⁻⁶個の誤り', '100万ビットに1個の誤り', '信号強度が10⁻⁶', '遅延が10⁻⁶秒'],
            correct: 1,
            explanation: 'BER = 10⁻⁶は、送信される100万ビットのうち平均1ビットに誤りが発生することを意味します。'
          },
          {
            question: 'エイリアシングを防ぐ方法は？',
            options: ['標本化周波数を下げる', 'ローパスフィルタを使用', '振幅を上げる', '位相を調整'],
            correct: 1,
            explanation: 'エイリアシングは標本化前にローパスフィルタで高周波成分を除去することで防げます。'
          },
          {
            question: 'SNRが高いときの通信品質は？',
            options: ['品質が悪い', '品質が良い', '変化なし', '不安定'],
            correct: 1,
            explanation: 'SNR（信号対雑音比）が高いほど雑音に対する信号の強度が大きく、通信品質は向上します。'
          },
          {
            question: 'PCM符号化の手順で正しいものは？',
            options: ['量子化→標本化→符号化', '標本化→量子化→符号化', '符号化→標本化→量子化', '標本化→符号化→量子化'],
            correct: 1,
            explanation: 'PCMでは、標本化（サンプリング）→量子化→符号化の順序で処理します。'
          }
        ]
      }
    ]
