...
- API: プログラム間のインターフェース`,
  ...
  {
    id: 4,
    title: '数学的基礎とアルゴリズム解析',
    sections: [
      {
        title: '計算量理論',
        content: `アルゴリズムの効率性を数学的に評価する理論：

**オーダー記法（Big-O記法）:**
- アルゴリズムの実行時間や空間使用量の上界を表現
- O(1): 定数時間 - 入力サイズに関係なく一定
- O(log n): 対数時間 - 二分探索など
- O(n): 線形時間 - リストの走査など
- O(n log n): 準線形時間 - 効率的なソートなど
- O(n²): 二次時間 - 単純なソートなど
- O(2ⁿ): 指数時間 - 組み合わせ最適化など

**その他の記法:**
- Ω(f(n)): 下界（最良の場合）
- Θ(f(n)): 平均的な場合（上界と下界が一致）
- o(f(n)): 上界（厳密に小さい）

**マスター定理:**
分割統治アルゴリズムの計算量を求める公式
T(n) = aT(n/b) + f(n) の形で表される漸化式の解法
...
- 再帰呼び出しのスタック使用量`,
        quizzes: [
          {
            question: 'n個の要素を持つ配列から最大値を見つけるアルゴリズムの計算量は？',
            options: ['O(1)', 'O(log n)', 'O(n)', 'O(n²)'],
            correct: 2,
            explanation: '配列のすべての要素を一度は確認する必要があるため、線形時間O(n)です。'
          },
          {
            question: 'バブルソートの最悪時間計算量は？',
            options: ['O(n)', 'O(n log n)', 'O(n²)', 'O(n³)'],
            correct: 2,
            explanation: 'バブルソートは二重ループで隣接要素を比較するため、最悪の場合O(n²)の時間がかかります。'
          },
          {
            question: '二分探索の時間計算量は？',
            options: ['O(1)', 'O(log n)', 'O(n)', 'O(n²)'],
            correct: 1,
            explanation: '二分探索では毎回探索範囲が半分になるため、O(log n)の時間計算量です。'
          },
          {
            question: 'マージソートの時間計算量は？',
            options: ['O(n)', 'O(n log n)', 'O(n²)', '最悪の場合のみO(n²)'],
            correct: 1,
            explanation: 'マージソートは分割統治により、最良・平均・最悪すべての場合でO(n log n)です。'
          },
          {
            question: 'ハノイの塔（n枚）を解く最小手数の計算量は？',
            options: ['O(n)', 'O(n²)', 'O(2ⁿ)', 'O(n!)'],
            correct: 2,
            explanation: 'ハノイの塔の解法は2ⁿ-1手必要で、計算量はO(2ⁿ)の指数時間です。'
          },
          {
            question: 'クイックソートの平均時間計算量は？',
            options: ['O(n)', 'O(n log n)', 'O(n²)', 'O(log n)'],
            correct: 1,
            explanation: 'クイックソートの平均的な場合の時間計算量はO(n log n)ですが、最悪の場合はO(n²)です。'
          },
          {
            question: 'フィボナッチ数列の単純な再帰実装の計算量は？',
            options: ['O(n)', 'O(n²)', 'O(2ⁿ)', 'O(n!)'],
            correct: 2,
            explanation: '単純な再帰実装では同じ値を何度も計算するため、指数時間O(2ⁿ)になります。'
          },
          {
            question: '動的プログラミングでフィボナッチ数列を求める計算量は？',
            options: ['O(1)', 'O(n)', 'O(n²)', 'O(2ⁿ)'],
            correct: 1,
            explanation: '動的プログラミングでは各値を一度だけ計算するため、線形時間O(n)で求められます。'
          },
          {
            question: 'ヒープソートの空間計算量は？',
            options: ['O(1)', 'O(log n)', 'O(n)', 'O(n log n)'],
            correct: 0,
            explanation: 'ヒープソートはin-placeソートアルゴリズムで、定数の補助空間O(1)しか使用しません。'
          },
          {
            question: 'Big-O記法で表される上界の意味は？',
            options: ['正確な実行時間', '最良の場合の時間', '最悪の場合の上限', '平均実行時間'],
            correct: 2,
            explanation: 'Big-O記法は、アルゴリズムの実行時間の上界（最悪の場合の上限）を表現します。'
          }
        ]
      },
      {
        title: '確率・統計とアルゴリズム',
        content: `確率・統計理論のアルゴリズムへの応用：

**確率的アルゴリズム:**
- ランダム化アルゴリズム: 乱数を使用してより効率的に問題を解決
- モンテカルロ法: 確率的シミュレーションによる近似解法
- ラスベガス法: 正確な答えを確率的時間で求める
- 期待実行時間の解析
...
- RSA暗号の数学的基盤`,
        quizzes: [
          {
            question: 'クイックソートでランダムピボットを選ぶ理由は？',
            options: ['常に最速になる', '最悪ケースを回避しやすくする', '実装が簡単', 'メモリ使用量を減らす'],
            correct: 1,
            explanation: 'ランダムピボットにより、意図的に作られた最悪ケースのデータでも平均的な性能を期待できます。'
          },
          {
            question: 'モンテカルロ法で円周率を求める原理は？',
            options: ['円の面積公式', '正方形内の点の分布', '三角関数の級数展開', '積分計算'],
            correct: 1,
            explanation: '単位正方形内にランダムな点を生成し、単位円内に入る点の割合から円周率を近似します。'
          },
          {
            question: 'n個の数値の中央値を求める効率的な方法は？',
            options: ['全てソートしてn/2番目を取る', 'クイックセレクト', 'ヒープを使用', 'ハッシュテーブル'],
            correct: 1,
            explanation: 'クイックセレクト（選択アルゴリズム）により、平均O(n)時間で中央値を求められます。'
          },
          {
            question: 'ベイズの定理 P(A|B) = P(B|A)P(A)/P(B) で、P(A)は何を表す？',
            options: ['条件付き確率', '事前確率', '事後確率', '尤度'],
            correct: 1,
            explanation: 'P(A)は事前確率で、観測前にAが起こると考えられる確率を表します。'
          },
          {
            question: 'ユークリッドの互除法でgcd(48, 18)を求める手順は？',
            options: ['48÷18=2余12, 18÷12=1余6, 12÷6=2余0, 答え6', '48÷18=2余12, 12÷18は不可', '48+18=66から逆算', '素因数分解して共通因数'],
            correct: 0,
            explanation: '互除法では大きい数を小さい数で割り、余りで割り続けて余りが0になったときの除数が最大公約数です。'
          },
          {
            question: '素数判定でミラー・ラビン法の特徴は？',
            options: ['確定的アルゴリズム', '確率的アルゴリズム', '線形時間', '指数時間'],
            correct: 1,
            explanation: 'ミラー・ラビン法は確率的素数判定法で、合成数を素数と誤判定する確率を低く抑えられます。'
          },
          {
            question: '標準偏差を計算するアルゴリズムで注意すべき点は？',
            options: ['必ず昇順ソートする', '浮動小数点の誤差', '負の値を除外', 'データ数が偶数の場合の処理'],
            correct: 1,
            explanation: '分散計算時の浮動小数点誤差を避けるため、適切な計算順序や精度の考慮が重要です。'
          },
          {
            question: 'ハッシュ関数の設計で重要な確率的性質は？',
            options: ['単調性', '連続性', '一様分散性', '可逆性'],
            correct: 2,
            explanation: 'ハッシュ値が一様に分散することで、衝突を最小化し効率的なデータアクセスが可能になります。'
          },
          {
            question: 'ブルームフィルタの偽陽性率を下げる方法は？',
            options: ['ビット配列を小さくする', 'ハッシュ関数を減らす', 'ビット配列を大きくする', 'データを事前ソート'],
            correct: 2,
            explanation: 'ビット配列のサイズを大きくするか、適切な数のハッシュ関数を使用することで偽陽性率を下げられます。'
          },
          {
            question: '機械学習の勾配降下法で学習率を設定する理由は？',
            options: ['計算量を減らす', '収束速度と安定性のバランス', 'メモリ使用量を制御', 'データの正規化'],
            correct: 1,
            explanation: '学習率が大きすぎると発散し、小さすぎると収束が遅くなるため、適切なバランスが重要です。'
          }
        ]
      },
      {
        title: '数値計算アルゴリズム',
        content: `コンピュータによる数値計算の基礎：
...
- 制約条件付き最適化`,
        quizzes: [
          {
            question: '浮動小数点計算で0.1 + 0.2 ≠ 0.3となる理由は？',
            options: ['プログラムのバグ', '2進表現での丸め誤差', 'オーバーフロー', 'アンダーフロー'],
            correct: 1,
            explanation: '0.1や0.2は2進数で正確に表現できないため、丸め誤差により0.3と厳密には等しくなりません。'
          },
          {
            question: 'ニュートン法で平方根を求める漸化式は？（√aを求める場合）',
            options: ['xₙ₊₁ = (xₙ + a/xₙ)/2', 'xₙ₊₁ = xₙ - a/xₙ', 'xₙ₊₁ = xₙ² - a', 'xₙ₊₁ = a/xₙ'],
            correct: 0,
            explanation: 'ニュートン法でf(x)=x²-aの根を求めると、xₙ₊₁ = (xₙ + a/xₙ)/2の漸化式が得られます。'
          },
          {
            question: 'ガウスの消去法の計算量は？',
            options: ['O(n²)', 'O(n³)', 'O(n⁴)', 'O(2ⁿ)'],
            correct: 1,
            explanation: 'n×n行列に対してガウスの消去法は約n³/3回の演算を要するため、O(n³)の計算量です。'
          },
          {
            question: '台形公式による数値積分の誤差は？',
            options: ['O(h)', 'O(h²)', 'O(h³)', 'O(h⁴)'],
            correct: 1,
            explanation: '台形公式の誤差は刻み幅hの2乗に比例するため、O(h²)の精度です。'
          },
          {
            question: 'LU分解の利点は？',
            options: ['計算量が少ない', '複数の右辺ベクトルに効率的', '常に安定', 'メモリ使用量が少ない'],
            correct: 1,
            explanation: 'LU分解により、異なる右辺ベクトルに対してO(n²)時間で解を求められます。'
          },
          {
            question: '条件数が大きい行列の特徴は？',
            options: ['計算が高速', '数値誤差に敏感', 'メモリ効率が良い', '必ず正定値'],
            correct: 1,
            explanation: '条件数が大きいと小さな入力誤差が大きな出力誤差になり、数値計算が不安定になります。'
          },
          {
            question: 'シンプソン公式の精度は？',
            options: ['1次精度', '2次精度', '3次精度', '4次精度'],
            correct: 3,
            explanation: 'シンプソン公式は3次多項式を正確に積分でき、一般には4次の精度を持ちます。'
          },
          {
            question: '黄金分割探索で使用する比率は？',
            options: ['1/2', '1/3', '(√5-1)/2', '2/3'],
            correct: 2,
            explanation: '黄金分割探索では黄金比φ=(√5-1)/2≈0.618を使用して効率的に最適点を探索します。'
          },
          {
            question: '多倍長演算が必要な場面は？',
            options: ['通常の四則演算', '暗号計算や数値解析', 'グラフィック処理', '文字列処理'],
            correct: 1,
            explanation: 'RSA暗号や高精度数値計算では、通常の整数や浮動小数点では精度不足のため多倍長演算が必要です。'
          },
          {
            question: '数値微分の前進差分公式は？',
            options: ["f'(x) ≈ (f(x+h) - f(x))/h", "f'(x) ≈ (f(x) - f(x-h))/h", "f'(x) ≈ (f(x+h) - f(x-h))/(2h)", "f'(x) ≈ f(x+h) - f(x-h)"],
            correct: 0,
            explanation: '前進差分では f\'(x) ≈ (f(x+h) - f(x))/h を使用して微分を近似計算します。'
          }
        ]
      }
    ]
